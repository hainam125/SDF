...
public class TutorialRaymarch : SceneViewFilter
{
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        _EffectMaterial.SetMatrix("_MatTorus_InvModel", MatTorus.inverse);

        _EffectMaterial.SetVector("_LightDir", _SunLight ? _SunLight.forward : Vector3.down);
        // pass frustum rays to shader
        _EffectMaterial.SetMatrix("_FrustumCornersES", GetFrustumCorners(CurrentCamera));
        _EffectMaterial.SetMatrix("_CameraInvViewMatrix", CurrentCamera.cameraToWorldMatrix);
        _EffectMaterial.SetVector("_CameraWS", CurrentCamera.transform.position);

        CustomGraphicsBlit(source, destination, _EffectMaterial, 0); // use given effect shader as image effect
    }
